...
func TestParser_ParseStatement(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			s: `CREATE RETENTION POLICY policy1 ON testdb DURATION 1h REPLICATION 2 SHARD DURATION 1s PARTITION 3`,
			stmt: &influxql.CreateRetentionPolicyStatement{
				Name:               "policy1",
				Database:           "testdb",
				Duration:           time.Hour,
		...
		{
			s: "ALTER NODES 'n1:8888','n2:7777' ENABLE LABELS 'c=d' MODE WO IPV6 'afaf:323:3ww:32' ENABLEIPV6 TRUE PORT 7000",
		...
		{
			s: "alter config on \"*\" maxBucketsN 5",
			stmt: &influxql.AlterConfigStatement{
				DB: "*",
		...
		{
			s: "show config on \"*\" ",
			stmt: &influxql.ShowConfigStatement{
				DB: "*",
			},
		},
		...
		{
			s: "drop config on \"*\"",
			stmt: &influxql.DropConfigStatement{
				DB: "*",
			},
		},
		// Errors
		...
		{s: `SHOW FOO`, err: `found FOO, expected CONTINUOUS, DATABASES, PROXIES, DIAGNOSTICS, FIELD, GRANTS, MEASUREMENT, MEASUREMENTS, QUERIES, RETENTION, SERIES, SHARD, SHARDS, STATS, SUBSCRIPTIONS, TAG, USERS, NODES, CONFIG at line 1, char 6`},
		...
		{s: `DROP FOO`, err: `found FOO, expected CONTINUOUS, DATABASE, MEASUREMENT, RETENTION, SERIES, SHARD, SUBSCRIPTION, USER, NODES, CONFIG at line 1, char 6`},
		...
		{s: `CREATE RETENTION POLICY policy1 ON testdb`, err: `expected DURATION`},
		...
		{s: `CREATE RETENTION POLICY policy1 ON testdb DURATION 1h`, err: `expected REPLICATION`},
		...
		{s: `ALTER`, err: `found EOF, expected RETENTION, NODES, CONFIG at line 1, char 7`},
		...
		{s: `ALTER RETENTION POLICY policy1 ON testdb`, err: `found EOF, expected DURATION, REPLICATION, SHARD, DEFAULT,PARTITION,KEY,NODES`},
